...
import { CotFoldToggle } from './chat/CotFoldToggle.tsx'
...
export function apply(ctx: Context): void {
  ...
  // UI-only turn folding: the session-header switch that toggles per-turn
  // folding (the fold grouping itself lives in ChatView). Registered like the
  // other additive header actions, so a folded fork never fights a second
